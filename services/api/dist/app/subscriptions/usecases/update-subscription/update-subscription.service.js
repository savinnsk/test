...
let UpdateSubscriptionService = class UpdateSubscriptionService {
    ...
    async execute(updateSubscriptionDto, id, client) {
        try {
            const subscription = await this.findSubscriptionService.execute(id);
            const currentPaymentConfig = this.getGatewayKey(client.paymentsConfigs, subscription.paymentGateway);
            const data = await this.send(subscription.paymentGateway, 'update-subscription', {
                data: Object.assign(Object.assign({}, updateSubscriptionDto), { payer: Object.assign(Object.assign({}, updateSubscriptionDto.payer), { customerId: subscription.payer.customerId }), subscriptionId: subscription.subscriptionId, paymentProfileId: subscription.paymentProfileId }),
                config: currentPaymentConfig,
            });
            if ((0, respose_is_error_type_1.responseIsAErrorType)(data)) {
                return data;
            }
            const subscriptionUpdated = await this.subscriptionsRepository.update(data, id);
            if (!subscriptionUpdated) {
                throw new common_1.BadRequestException('Subscription cant be updated.');
            }
...
UpdateSubscriptionService = __decorate([
